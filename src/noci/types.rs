...
pub trait NOCIScalar: StateScalar + From<f64> + Scalar<Real = f64> + ERIScalar {
	...
	fn einsum_ba_ab(g: &Array2<Self>, h: &Array2<Self>) -> Self;
	...
	/// # Returns
	/// - `Self`: Contracted scalar.
	fn einsum_ba_ab_realop(g: &Array2<Self>, h: &Array2<f64>) -> Self;
	...
	/// # Returns
	/// - `Self`: Contracted scalar.
	fn einsum_ba_abcd_cd(g: &Array2<Self>, t: &Array4<Self>, h: &Array2<Self>) -> Self;
	...
	/// # Returns
	/// - `Self`: Contracted scalar.
	fn einsum_ba_abcd_cd_realop(g: &Array2<Self>, t: &Array4<f64>, h: &Array2<Self>) -> Self;
}
...
impl<'a, T: NOCIScalar> NOCIData<'a, T> {
    ...
    pub fn new(ao: &'a AoData, basis: &'a [DetState<T>], input: &'a Input, tol: f64, wicks: Option<&'a WicksView<T>>) -> Self {
    ...
    pub fn withmocache(mut self, mocache: &'a [MOCache<T>]) -> Self {
...
impl<'a, T: NOCIScalar> FockData<'a, T> {
    ...
    pub(crate) fn new(fock_mocache: &'a [FockMOCache<T>], fa: &'a Array2<T>, fb: &'a Array2<T>) -> Self {
...
impl<'a, T: NOCIScalar> DetPair<'a, T> {
	...
	pub(crate) fn new(ldet: &'a DetState<T>, gdet: &'a DetState<T>) -> Self {
        Self {ldet, gdet}
    }
}
