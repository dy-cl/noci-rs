...
pub fn occ_coeffs<T: NOCIScalar>(c: &Array2<T>, occ: u128) -> Array2<T> {
...
pub(in crate::noci) fn build_s_pair<T: NOCIScalar>(l_c_occ: &Array2<T>, g_c_occ: &Array2<T>, s_munu: &Array2<f64>, tol: f64) -> Pair<T> {
    ...
    let s = if zeros.is_empty() {phase * <T as From<f64>>::from(prod)} else {<T as From<f64>>::from(0.0)};
...
fn calculate_codensity_p_pair<T: NOCIScalar>(l_tilde_c_occ: &Array2<T>, g_tilde_c_occ: &Array2<T>, i: usize) -> Array2<T> {
...
fn calculate_codensity_w_pair<T: NOCIScalar>(l_tilde_c_occ: &Array2<T>, g_tilde_c_occ: &Array2<T>, tilde_s: &Array1<f64>, tol: f64) -> Array2<T> {
...
fn pair_density<T: NOCIScalar>(pair: &Pair<T>, nao: usize) -> Array2<T> {
...
pub fn noci_density<T: NOCIScalar>(ao: &AoData, states: &[DetState<T>], c: &Array1<T>, tol: f64) -> (Array2<T>, Array2<T>) {
	let nao = ao.h.nrows();
	let nst = states.len();
	(0..nst).into_par_iter().map(|i| {
		...
		(da_loc, db_loc)
	}).reduce(|| (Array2::<T>::zeros((nao, nao)), Array2::<T>::zeros((nao, nao))), |(mut da_a, mut db_a), (da_b, db_b)| {
		da_a += &da_b; db_a += &db_b; (da_a, db_a)
	})
}
...
pub(in crate::noci) fn one_electron<T: NOCIScalar>(o: &Array2<f64>, pair: &Pair<T>) -> T {
    let fac = pair.phase * <T as From<f64>>::from(pair.s_red);
    match pair.zeros.len() {
...
/// # Returns:
/// - `T`: One-electron matrix element for the determinant pair, before external determinant excitation phase.
pub(in crate::noci) fn one_electron_scalar<T: NOCIScalar>(o: &Array2<T>, pair: &Pair<T>) -> T {
    let fac = pair.phase * <T as From<f64>>::from(pair.s_red);
    match pair.zeros.len() {
        // With no zeros (s_i != 0 for all i) we use munu_w.
...
pub(in crate::noci) fn two_electron_same<T: NOCIScalar>(o: &Array4<f64>, pair: &Pair<T>) -> T {
    let fac = pair.phase * <T as From<f64>>::from(pair.s_red);
    match pair.zeros.len() {
        // With no zeros (s_i != 0 for all i) we use munu_w on both sides.
        0 => <T as From<f64>>::from(0.5) * fac * T::einsum_ba_abcd_cd_realop(pair.w.as_ref().unwrap(), o, pair.w.as_ref().unwrap()),
        // With 1 zero (s_i = 0 for one index i) we use P_i on one side.
        1 => fac * T::einsum_ba_abcd_cd_realop(pair.p_i.as_ref().unwrap(), o, pair.w.as_ref().unwrap()),
        // with 2 zeros (s_i, s_j = 0 for two indices i, j) we use P_i on one side and P_j on the other.
        2 => fac * T::einsum_ba_abcd_cd_realop(pair.p_i.as_ref().unwrap(), o, pair.p_j.as_ref().unwrap()),
...
pub(in crate::noci) fn two_electron_diff<T: NOCIScalar>(o: &Array4<f64>, pa: &Pair<T>, pb: &Pair<T>) -> T {
    ...
    match (pa.zeros.len(), pb.zeros.len()) {
        ...
        (0, 0) => fa * fb * T::einsum_ba_abcd_cd_realop(pa.w.as_ref().unwrap(), o, pb.w.as_ref().unwrap()),
        // With one zero in beta spin only we use W^a and P_i^b.
        (0, 1) => fa * fb * T::einsum_ba_abcd_cd_realop(pa.w.as_ref().unwrap(), o, pb.p_i.as_ref().unwrap()),
        // With one zero in alpha spin only we use P_i^a and W_b.
        (1, 0) => fa * fb * T::einsum_ba_abcd_cd_realop(pa.p_i.as_ref().unwrap(), o, pb.w.as_ref().unwrap()),
        // With one zero in both spins we use P_i^a and P_i^b.
        (1, 1) => fa * fb * T::einsum_ba_abcd_cd_realop(pa.p_i.as_ref().unwrap(), o, pb.p_i.as_ref().unwrap()),
        // Otherwise the matrix element is zero.
        _ => <T as From<f64>>::from(0.0),
    }
}
