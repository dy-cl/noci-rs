...
pub(crate) fn calculate_s_pair<T: NOCIScalar>(data: &NOCIData<'_, T>, pair: DetPair<'_, T>, scratch: Option<&mut WickScratchSpin<T>>) -> T {
	time_call!(crate::timers::noci::add_calculate_s_pair, {
		let ldet = pair.ldet;
		let gdet = pair.gdet;

		if ldet.parent == gdet.parent {
			let mocache = data.mocache.expect("Orthogonal overlap matrix elements require mocache.");
...
pub(in crate::noci) fn calculate_s_pair_orthogonal<T: NOCIScalar>(ldet: &DetState<T>, gdet: &DetState<T>) -> T {
...
/// # Returns:
/// - `T`: Overlap matrix element between `ldet` and `gdet`.
fn calculate_s_pair_naive<T: NOCIScalar>(data: &NOCIData<'_, T>, ldet: &DetState<T>, gdet: &DetState<T>) -> T {
...
fn calculate_s_pair_wicks<T: NOCIScalar>(ldet: &DetState<T>, gdet: &DetState<T>, wicks: &WicksView<T>, scratch: &mut WickScratchSpin<T>) -> T {
