...
fn hermitian_orthonormal_error<T: NOCIScalar>(c: &Array2<T>, s: &Array2<f64>) -> f64 {
    ...
    for i in 0..ov.nrows() {
        for j in 0..ov.ncols() {
            let target = if i == j {<T as From<f64>>::from(1.0)} else {<T as From<f64>>::from(0.0)};
...
pub fn build_mo_cache<T: NOCIScalar>(ao: &AoData, parents: &[DetState<T>], tol: f64) -> Vec<MOCache<T>> {
    time_call!(crate::timers::noci::add_build_mo_cache, {
        parents.iter().map(|st| {
            ...
            let eri_aa_asym = eri_ao2mo_hermitian_as(&ao.eri_asym, ca, ca, ca, ca).as_standard_layout().to_owned();
            let eri_bb_asym = eri_ao2mo_hermitian_as(&ao.eri_asym, cb, cb, cb, cb).as_standard_layout().to_owned();
            let eri_ab_coul = eri_ao2mo_hermitian_as(&ao.eri_coul, ca, ca, cb, cb).as_standard_layout().to_owned();

            let hermitian_orthonormal = hermitian_orthonormal_error(ca, &ao.s) < tol && hermitian_orthonormal_error(cb, &ao.s) < tol;

            MOCache {ha, hb, eri_aa_asym, eri_bb_asym, eri_ab_coul, hermitian_orthonormal}
        }).collect()
    })
}
...
pub fn build_fock_mo_cache<T: NOCIScalar>(fa: &Array2<T>, fb: &Array2<T>, parents: &[DetState<T>], s: &Array2<f64>, tol: f64) -> Vec<FockMOCache<T>> {
    time_call!(crate::timers::noci::add_build_fock_mo_cache, {
        parents.iter().map(|st| {
            ...
            let hermitian_orthonormal = hermitian_orthonormal_error(ca, s) < tol && hermitian_orthonormal_error(cb, s) < tol;

            FockMOCache {fa: fa_mo, fb: fb_mo, hermitian_orthonormal}
        }).collect()
    })
}
