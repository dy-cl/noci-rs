...
pub(crate) fn rdm3<T: NOCIScalar>(
    ...
    for x in 0..data.basis.len() {
        for w in 0..data.basis.len() {
            ...
            let (sxw, gxw) = if data.input.wicks.enabled && data.input.wicks.compare {
                let ((sxw, gxw), d) = compare_rdm3_pair_wicks_naive(
                    data,
                    pair,
                    active,
    ...
    if data.input.wicks.enabled && data.input.wicks.compare {
        println!(
            "Total naive–wicks discrepancy (active spin-free 3-RDM): {:.6e}",
            td
        );
    }
...
/// - `scratch`: Scratch space for Wick's calculations.
/// # Returns:
/// - `((T, RDM3<T>), f64)`: Pair overlap and active-space spin-free 3-RDM from Wick's
