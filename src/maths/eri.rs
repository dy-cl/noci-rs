...
/// Transform ERIs from AO to complex MO basis without complex conjugation as:
///     (pq|rs) = \sum_{\mu\nu\lambda\sigma} (\mu\nu|\lambda\sigma) C_{\mu, p} C_{\nu, q} C_{\lambda, r} C_{\sigma, s}.
/// This is the complex-bilinear, holomorphic-style transform.
/// # Arguments:
/// - `eri`: AO basis ERIs.
/// - `c_mu_p`: MO coefficients C_{\mu, p}.
/// - `c_nu_q`: MO coefficients C_{\nu, q}.
...
pub fn eri_ao2mo_complex_bilinear(eri: &Array4<f64>, c_mu_p: &Array2<Complex64>, c_nu_q: &Array2<Complex64>, c_lam_r: &Array2<Complex64>, c_sig_s: &Array2<Complex64>) -> Array4<Complex64> {
    let nbas = c_mu_p.nrows();
    let nmo_p = c_mu_p.ncols();
    let nmo_q = c_nu_q.ncols();
    let nmo_r = c_lam_r.ncols();
