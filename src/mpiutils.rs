...
impl Sharedffi {
    ...
    pub fn allocate(
        ...
        unsafe {
            ...
            mpi::ffi::MPI_Comm_split_type(
                world.as_raw(),
                mpi::ffi::MPI_COMM_TYPE_SHARED as i32,
                0,
            ...
            mpi::ffi::MPI_Win_allocate_shared(
                bytes,
                unit,
                mpi::ffi::RSMPI_INFO_NULL,
                shared_comm,
                (&mut base as *mut *mut c_void).cast::<c_void>(),
                &mut win,
            );
            ...
            // in this case).
            mpi::ffi::MPI_Win_shared_query(
                win,
                0,
                &mut qsize,
                &mut qdisp,
                (&mut qbase as *mut *mut c_void).cast::<c_void>(),
            );

            Self {
                shared_comm,
                win,
                base: qbase as *mut u8,
                nbytes: qsize as usize,
                shared_rank,
                shared_size,
            }
        }
    ...
    pub fn barrier(&self) {
        unsafe {
            mpi::ffi::MPI_Barrier(self.shared_comm);
        }
    }
}
