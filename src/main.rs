...
use noci_rs::noci::{MOCache, NOCIData, NOCIScalar};
...
struct PostSCFData<'a> {
    ...
    noci_reference_basis: &'a [SCFState],
    /// MO-basis one and two-electron integral caches.
    mocache: &'a [MOCache<f64>],
...
fn run(
    ...
    if run_holomorphic {
        ...
        let wicks_shared: Option<WicksShared<Complex64>> =
        ...
        if irank == 0 {
            ...
            if holomorphic_noci_refs
                && (input.det.is_some() || input.snoci.is_some() || input.qmc.is_some())
            {
                println!(
                    "Holomorphic NOCI states not currently supported for deterministic or stochastic propagation and SNOCI. Returning after SCF and reference NOCI."
                );
            }
        }
    ...
    if irank == 0 {
        ...
        states = run_scf(&ao, input, prev_states);
    ...
    // computation happens on only world rank 0 (with rayon). If multiple shared memory regions are
    // used multiple copies of the intermediates will be computed.
    let mut wicks_shared: Option<WicksShared<f64>> =
        time_call!(crate::timers::general::add_build_wicks_shared, {
            if input.wicks.enabled || input.wicks.compare {
                if irank == 0 {
                    println!("{}", "=".repeat(100));
    ...
    if irank == 0 {
        ...
        if input.det.is_some() {
            let e_det = run_qmc_deterministic_noci(&post, input, &c0, wicks_view);
    ...
    if input.qmc.is_some() {
        ...
        let e_qmc = run_qmc_stochastic_noci(&post, input, &c0, world, wicks_view);
...
fn run_reference_noci<T: NOCIScalar>(
    ao: &AoData,
    input: &Input,
    states: &[DetState<T>],
    tol: f64,
...
) -> (Vec<DetState<T>>, f64, Vec<T>) {
    time_call!(crate::timers::general::add_run_reference_noci, {
        let mut noci_reference_basis: Vec<DetState<T>> =
...
fn run_snoci(
    post: &PostSCFData<'_>,
    initial_space: &[SCFState],
    input: &Input,
    wicks: Option<&mut WicksShared<f64>>,
    world: &impl Communicator,
) -> (f64, f64) {
...
fn print_report(
    ...
    fn print_relative_counter(
        label: &str,
        counter: noci_rs::timers::Counter,
        parent: noci_rs::timers::Counter,
        indent: usize,
    ) {
    ...
    if !res.hstates.is_empty() {
        ...
        println!(
            "Complex h-SCF/reference NOCI run completed; det, SNOCI, and QMC reporting is skipped."
        );
        println!("{}", "=".repeat(100));
        return;
    }
