...
use noci_rs::timers;
use noci_rs::{DetState, HSCFState, PostSCFData, SCFState};
...
fn run(
    ...
    if run_holomorphic {
        ...
        let mut wicks_shared: Option<WicksShared<Complex64>> =
        ...
        if irank == 0 {
            ...
            if holomorphic_noci_refs && (input.det.is_some() || input.qmc.is_some()) {
                println!(
        ...
        // SNOCI/NOCI-PT2 must be entered by every rank because the PT2 matvec uses MPI collectives.
    ...
    // Deterministic propagation remains rank-0 only.
    if irank == 0 && input.det.is_some() {
        let wicks_view = wicks_shared.as_ref().map(|ws| ws.view());
