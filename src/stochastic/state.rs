...
pub(in crate::stochastic) struct ThreadPropagation {
    ...
    /// Excitation generation samples.
    pub(in crate::stochastic) samples: Vec<f64>,
    /// Overlap-transformed population increments generated by this thread.
    pub(in crate::stochastic) overlap: Vec<(usize, f64)>,
    /// Thread local RNG.
...
impl ThreadPropagation {
    ...
    /// Clear overlap-transformed population increments while retaining allocated storage.
    /// # Arguments:
    /// - `self`: Per-thread propagation storage to clear.
    /// # Returns
    /// - `()`: Clears overlap increments without releasing their allocation.
    pub(in crate::stochastic) fn clear_overlap(&mut self) {
        self.overlap.clear();
    }
