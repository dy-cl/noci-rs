...
/// Report-block derivative of the pre-overlap change with respect to the DirectOverlap shift.
/// The stored vector is `B = \partial\Delta/\partial E_s = \sum_a dt S\tilde N^{(a)}`.
...
impl ShiftTangent {
    ...
    pub(in crate::stochastic) fn new(ndets: usize) -> Self {
        // The tangent is accumulated densely because `B = \sum_a dt S\tilde N^{(a)}` occupies
        // almost the complete determinant basis before report-level FRI.
        Self {
    ...
    pub(in crate::stochastic) fn prepare(
        ...
        // Allocate N_det storage lazily because only DirectOverlap uses the shift tangent.
...
pub(in crate::stochastic) struct ThreadPropagation {
    ...
    pub(in crate::stochastic) remote: Vec<(usize, PopulationUpdate)>,
    /// Report-level DirectOverlap shift tangent accumulated by this worker.
...
impl ThreadPropagation {
    ...
    pub(in crate::stochastic) fn with_sizes(
        ...
        // storage. DirectOverlap expands it on first propagation and retains allocation thereafter.
        Self {
            local: Vec::new(),
            remote: Vec::new(),
    ...
    /// ownership logic. For DirectOverlap, same realised overlap elements also supply
    ...
    pub(in crate::stochastic) fn resolve_batched_spawning(
        ...
        for i in 0..self.spawn_requests.len() {
            ...
            let raw = if accumulate_shift_tangent {
                // Both DirectOverlap physical spawning and shift tangent contain
